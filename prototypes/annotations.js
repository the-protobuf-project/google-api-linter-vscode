...
const RE_XFIELD =
	/^\s*(?:(optional|repeated)\s+)?([A-Za-z_][\w.]*)\s+([a-z_]\w*)\s*=\s*(\d+)\s*;/;
			...
			for (let j = i + 1; j < lines.length && depth > 0; j++) {
				...
				if (f)
					fields.push({
						...
						number: Number(f[4]),
						repeated: f[1] === "repeated",
						doc: splitDoc(leadingComment(lines, j)).prose,
					});
			}
			messages.set(pkg ? `${pkg}.${msg[1]}` : msg[1], {
				fields,
				file: rel,
				line: i + 1,
			});
...
function walk(d, o = []) {
	for (const e of fs.readdirSync(d, { withFileTypes: true })) {
		...
		else if (e.name.endsWith(".proto")) o.push(p);
	}
	return o;
}
...
if (require.main === module) {
	...
	console.log(
		`discovered ${annotations.length} annotations across ${byPkg.size} namespaces`,
	);
	...
	for (const [ns, list] of [...byPkg].sort()) {
		...
		console.log(
			ns.padEnd(26) + String(list.length).padStart(4) + "   " + targets,
		);
	}
}
