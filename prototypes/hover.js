...
const TARGET_LABEL = {
  File: 'file', Message: 'message', Field: 'field', Oneof: 'oneof',
  Enum: 'enum', EnumValue: 'enum value', Service: 'service', Method: 'rpc',
};
...
function hover(a) {
  const L = [];
  L.push(`### \`(${a.fqn})\``);
  L.push(`**${TARGET_LABEL[a.target]}** option · \`${a.type}\` · field ${a.number}`);
  L.push('');
  if (a.doc) { L.push(a.doc); L.push(''); }

  const body = messages.get(`${a.fqn.split('.').slice(0, -1).join('.')}.${a.type}`) ||
  ...
  if (a.example) { L.push('```proto'); L.push(a.example); L.push('```'); L.push(''); }
...
if (!a) { console.log(`not found: ${want}`); process.exit(1); }
console.log(hover(a));
console.log('\n' + '═'.repeat(70) + '\n');
...
for (const x of annotations.sort((p,q)=>p.number-q.number))
  console.log(`  (${x.fqn})`.padEnd(28) + `[${TARGET_LABEL[x.target]}]`.padEnd(14) + `${x.type}  #${x.number}`);
