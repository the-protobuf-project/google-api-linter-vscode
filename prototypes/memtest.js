...
console.log(
	`walk: ${files.length} files in ${Date.now() - t}ms, rss +${mb(rss() - base)} MB`,
);
...
console.log(
	`read+parse: ${mb(bytes)} MB of text, ${symbolCount} symbols in ${Date.now() - t}ms`,
);
console.log(
	`rss after full index build: ${mb(afterParse)} MB total, +${mb(afterParse - base)} MB over baseline`,
);
